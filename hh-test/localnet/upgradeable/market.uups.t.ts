...
describe("Market Tests (UUPS)", () => {
    ...
    it("Test 1: Basic Deal Flow with UUPS Proxy Upgrade", async () => {
...
const test1 = async (testName: string) => {
    ...
    const { deployer, client, storageProvider } = await utils.performGeneralSetup()

    dbg(`Deploying contracts... (market and helper)`)

    const MarketContractFactory = (await ethers.getContractFactory("MarketApiUUPSUpgradeableTest", deployer.eth.signer)) as any

    const marketContract = (await upgrades.deployProxy(MarketContractFactory, [], {
        kind: "uups",
    ...
    for (const stage of ["beforeUpgrade", "afterUpgrade"]) {
        ...
        if (stage == "beforeUpgrade") {
            ...
            const MarketContractFactoryV2 = (await ethers.getContractFactory("MarketApiUUPSUpgradeableV2Test", deployer.eth.signer)) as any
