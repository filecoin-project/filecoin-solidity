...
describe("Market Tests (Beacon)", () => {
    ...
    it("Test 1: Basic Deal Flow with Beacon Proxy Upgrade", async () => {
...
const test1 = async (testName: string) => {
    ...
    const beacon = await upgrades.deployBeacon(MarketContractFactory)
    await utils.defaultTxDelay()

    dbg(`Market Beacon Deployed.`)

    const instance = await upgrades.deployBeaconProxy(beacon, MarketContractFactory, [])
    await utils.defaultTxDelay()

    const market = { eth: { contract: instance, address: await instance.getAddress() }, fil: { address: "" } }
