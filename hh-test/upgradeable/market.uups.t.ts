...
describe("Market contract - UUPS Proxy Upgrade", function () {
    it("Should publish a deal", async function () {
        ...
        const MarketContractFactory = await ethers.getContractFactory("MarketApiUUPSUpgradeableTest", deployer)
        ...
        const MarketContractFactoryV2 = await ethers.getContractFactory("MarketApiUUPSUpgradeableV2Test", deployer)
        await upgrades.upgradeProxy(marketContract.address, MarketContractFactoryV2, {
            kind: "uups",
            unsafeAllow: ["delegatecall"],
        })

        console.log(`Upgrade perfomed`)
