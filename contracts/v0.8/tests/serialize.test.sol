...
contract FilAddressSerialize is Test {
    ...
    function test_1() external returns (CommonTypes.FilAddress memory addr1, CommonTypes.FilAddress memory addr2) {
        addr1 = CommonTypes.FilAddress({data: abi.encodePacked([uint8(0), 0x04, 0x22])});
        ...
        addr2 = CommonTypes.FilAddress({data: abi.encode([0x66])});
